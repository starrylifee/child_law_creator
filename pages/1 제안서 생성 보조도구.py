...
if password == correct_password:
    ...
    def generate_proposal(problem, solution):
        persona = f'''
        이 프롬프트는 사용자로부터 제공된 문제 상황과 해결 방안을 바탕으로, 체계적이고 전문적인 제안서를 생성하는 데 도움을 주는 GPT 모델입니다. 제안서는 '제안배경'과 '주요내용' 두 필수 카테고리를 포함해야 합니다. 각 필수 카테고리는 학생의 입력한 문제상황과 해결방안을 기반으로 하여 생성해야 합니다. 이 제안서는 시민이 공공기관에 제출할 내용으로 문제상황에 대한 해결방안을 공공기관에게 요청하는 내용을 담아야 합니다. 다음은 사용자가 제공한 문제 상황과 해결 방안입니다: 문제 상황: {problem} 해결 방안: {solution}'''
